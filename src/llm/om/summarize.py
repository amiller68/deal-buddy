import anthropic
import json
from src.database.models.om import PropertyType
from src.llm.utils.json_helpers import parse_llm_json_response
import logging

PROPERTY_TYPES = ", ".join(PropertyType.__members__.values())
...
5. Sqaure Footage (if known, as an integer)
6. Total Units (if known, as an integer)
7. Property Type (if known, as a string, one of the following: {property_types})
...
Provide your response in JSON format with keys address, title, description, and summary. Important: The summary should be a single line of text with no line breaks."""
    ...
    try:
        response = anthropic_client.messages.create(
            ...
            max_tokens=1500,
            messages=[{"role": "user", "content": prompt.format(pdf_text=pdf_text, property_types=PROPERTY_TYPES)}],
        )
        ...
        required_keys = ["address", "title", "description", "summary", "square_feet"]
        return parse_llm_json_response(content.text, keys=required_keys)

    except json.JSONDecodeError as e:
        logger.error(f"Failed to decode JSON response: {e}")
        raise
    except ValueError as e:
        logger.error(f"Value error in generate_summary: {e}")
        raise
